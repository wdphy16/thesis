...
In this appendix, we present the derivation of the SR formula in \cref{eq:sr-eq}. Substituting \cref{eq:psi-dtau-proj} into \cref{eq:sr-eq-1,eq:sr-eq-2}, and ignoring $O(\Delta \tau^2)$ terms, we have
\begin{align}
...
\end{align}
where $\vf$ is the gradient in \cref{eq:vmc-grad-real}, $\hat{\vD}$ is the vector of all $\{\hat{D}_i\}$, and the transpose $\hat{\vD}^\transpose$ is defined on the index $i$ in the parameter space, unlike the adjoint $\hat{\vD}^\dagger$ defined on the index $\vs$ in the configuration space.
