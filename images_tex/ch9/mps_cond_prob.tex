...
\tikzstyle{site}=[rectangle, rounded corners, draw=mplblue!80!black, fill=mplblue!20!white, minimum size=1.2cm, inner sep=0]
...
\node [site] (M0) at (0, 1) {$\arr{M}^{(0)}$};
\node [site] (M1) at (1, 1) {$\arr{M}^{(1)}$};
\node [site] (M2) at (2, 1) {$\arr{M}^{(2)}$};
\node [site] (M3) at (3, 1) {$\arr{M}^{(3)}$};
\node [site] (M4) at (4, 1) {$\arr{M}^{(4)}$};
\node [site] (M5) at (5, 1) {$\arr{M}^{(5)}$};

\node [site] (M0d) at (0, 0) {$\arr{M}^{(0) \dagger}$};
\node [site] (M1d) at (1, 0) {$\arr{M}^{(1) \dagger}$};
\node [site] (M2d) at (2, 0) {$\arr{M}^{(2) \dagger}$};
\node [site] (M3d) at (3, 0) {$\arr{M}^{(3) \dagger}$};
\node [site] (M4d) at (4, 0) {$\arr{M}^{(4) \dagger}$};
\node [site] (M5d) at (5, 0) {$\arr{M}^{(5) \dagger}$};
...
\node [draw=mplgreen, dashed, fit=(s0) (M2)] (h) {};
\node [draw, dashed, fit=(M3) (M5d) (t6)] (gamma) {};
...
\node [annot, left, mplmagenta] at ($(si2) + (-0.25, 0)$) {$s_2$};
\node [annot, above, text=mplgreen] at (h.north) {$\arr{h}^{(2)}$};
\node [annot, below] at (gamma.south) {$\arr{\gamma}^{(2)}$};
