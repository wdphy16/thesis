...
\node (Mm) at (-0.6, 0) {};
\node [site] (M0) at (0, 0) {$\arr{M}^{(0)}\,\arr{\gamma}^{(0)}$};
\node [site] (M1) at (1, 0) {$\arr{M}^{(1)}\,\arr{\gamma}^{(1)}$};
\node [site] (M2) at (2, 0) {$\arr{M}^{(2)}\,\arr{\gamma}^{(2)}$};
\node (M3) at (2.6, 0) {};

\node [site, draw=black, fill=white] (prod) at ($(M1) + (0, 2)$) {$\prod \sqrt{\phantom{x}}$};

\node [annot, above, text=mplgreen] (hm) at ($(Mm)!.2!(M0)$) {$\arr{h}^{(-1)}$};
\node [annot, above, text=mplgreen] (h0) at ($(M0)!.5!(M1)$) {$\arr{h}^{(0)}$};
\node [annot, above, text=mplgreen] (h1) at ($(M1)!.5!(M2)$) {$\arr{h}^{(1)}$};
\node [annot, above, text=mplgreen] (h2) at ($(M2)!.8!(M3)$) {$\arr{h}^{(2)}$};

\node [annot, text=mplmagenta] (si0) at ($(M0) + (0, -1)$) {$s_0$};
\node [annot, text=mplmagenta] (si1) at ($(M1) + (0, -1)$) {$s_1$};
\node [annot, text=mplmagenta] (si2) at ($(M2) + (0, -1)$) {$s_2$};

\node [annot, text=mplorange] (p0) at ($(M0) + (0, 1)$) {$p(s_0)$};
\node [annot, text=mplorange] (p1) at ($(M1) + (0, 1)$) {$p(s_1 | s_0)$};
\node [annot, text=mplorange] (p2) at ($(M2) + (0, 1)$) {$p(s_2 | s_0, s_1)$};
