...
After the above introduction to the general framework of VMC, in the following we discuss some common choices of the variational ansatz for quantum spin-$1/2$ systems that are of interest in this thesis.

\section{Variational ansatzes for quantum systems}

\subsection{Jastrow ansatz}

The Jastrow ansatz~\cite{jastrow1955many} was originally proposed to model the interactions between particles depending on their relative distances. In the context of spin systems, it is a bilinear form of the input configuration~\cite{huse1988simple}:
\begin{equation}
\ln \psi(\vs) = \frac{1}{2} \vs^\transpose \mW \vs,
\end{equation}
where $\mW$ is an $N \times N$ matrix of tunable parameters. Despite its simplicity, it is capable of modeling two-spin correlations and providing some initial insights about the system. The symmetries of the system can be imposed in the structure of $\mW$.

\subsection{Gutzwiller projected states}

Quantum many-body systems can exhibit a plethora of collective and emergent behaviors beyond two-spin correlations, such as spinons and fractional excitations in spin liquids~\cite{misguich2010quantum, balents2010spin}. Although we mainly focus on spin systems in this thesis, an approach to study these systems is to transform the Hamiltonian of spins to an equivalent Hamiltonian of fermions, then it is possible to construct highly entangled states using the anti-commutation structure of the fermions, which produce the necessary many-body correlations to model the aforementioned intricate behaviors. These introduced fermions are known as partons, namely Abrikosov fermions~\cite{abrikosov1965electron} in the following case. The transformation is given by
\begin{equation}
...
Slater determinant~\cite{slater1929theory}

Pair-product state

Jastrow correlator

RBM correlator~\cite{nomura2017restricted, ferrari2019neural, nomura2021dirac}

We refer to Ref.~\cite{ferrari2019static} as a self-contained introduction to this technique.

\subsection{Neural quantum states (NQS)}

RBM~\cite{carleo2017solving, nomura2021dirac}

CNN~\cite{choo2019two}

ClebschTree~\cite{vieijra2021many}
