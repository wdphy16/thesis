...
\label{eq:sr-eq}
\end{equation}
where $\vf$ is the gradient in \cref{eq:vmc-grad-real}, commonly known as the ``force'' in the context of VMC, and $\mS$ is a $n_\text{p} \times n_\text{p}$ matrix define by
\begin{equation}
...
In practice, we usually need to estimate $\mS$ using Monte Carlo sampling:
\begin{align}
\bbE_{\text{MC}}[\mS] &= \frac{1}{M} \sum_{i = 1}^M
...
\vs^{(i)} \sim q,
\end{align}
where $M$ is the number of samples, and usually $M < n_\text{p}$. In this case, a problem occurs that each term $\vD(\vs) \vD(\vs)^\transpose$ is a rank-$1$ matrix, so the rank of $\bbE_{\text{MC}}[\mS]$ is at most $M$, and it cannot be inverted in \cref{eq:sr}. A common remedy is to add a diagonal term to $\mS$ before inverting it:
\begin{equation}
\theta_{t + 1} \gets \theta_t - \gamma \left. (\mS + \lambda \mI)^{-1} \vf \right|_{\theta = \theta_t}.
\label{eq:sr-ds}
\end{equation}
...
Recently, a different approach to invert $\mS$ has been proposed in Refs.~\cite{chen2023efficient, rende2024simple}. It has been proven that the estimated update to the parameters can be expressed as
\begin{equation}
\theta_{t + 1} \gets \theta_t - \gamma \left. (\mX \mX^\transpose + \lambda \mI)^{-1} \mX \tilde{\vf} \right|_{\theta = \theta_t},
\label{eq:minsr}
\end{equation}
...
\end{equation}
and $\tilde{\vf}$ is the estimated force. This method is named MinSR in Ref.~\cite{chen2023efficient}, and it is gradually becoming the default implementation of SR when $M < n_\text{p}$. Note that even if $\mX \mX^\transpose$ is full-rank, the diagonal shift $\lambda$ is usually still needed to improve the stability, because the limited number of samples can cause significant noise in $\mS$. It is worth mentioning that previously the large size of $\mS$ requires us to use iterative solvers for the linear system in \cref{eq:sr-eq}, which avoid storing all the $n_\text{p} \times n_\text{p}$ in $\mS$. As the size of the matrix to reduce is significantly smaller in MinSR, we can use exact solvers instead, which enable more parallelization on GPUs, and have fewer hyperparameters.

However, $\mX \mX^\transpose$ can still be singular if the expressiveness of the ansatz is limited, which is indicated by degeneracy in the basis vectors of the locally linearized variational space in \cref{eq:psi-dtau-proj}. Worse still, the magnitude of $\psi(\vs)$ can be exponentially suppressed for many $\vs$, which causes numerically zero eigenvalues in $\mS$. These zero or near-zero eigenvalues correspond to zero or near-zero entries in $\vf$ in principle, but the estimated linear system can contain noise that leads to a pathological solution of $\mS^{-1} \vf$. Therefore, extra care should be taken when inverting $\mS$. Besides the diagonal shift, it has been proposed to truncate the near-zero eigenvalues before the pseudoinverse, either with a fixed threshold or using the signal-to-noise ratio (SNR) of $\mS$~\cite{schmitt2020quantum, chen2023efficient}. Alternatively, Ref.~\cite{goldshlager2024kaczmarz} has proposed a running estimation of $\mS^{-1}$ over multiple optimization steps, which increases the effective sample size compared to inverting it in each step.

It has been shown that SR is necessary to obtain correct solutions to non-stoquastic systems in many cases~\cite{carleo2017solving, choo2019two}, and the simple SGD or its variants such as Adam fail to converge to the ground state. This success is usually attributed to the incorporation of the local geometry in SR, which smoothens the highly non-convex energy landscape. In the ideal case where the estimations of $\vf$ and $\mS$ are exact, the ansatz has sufficiently high expressiveness, and the learning rate $\gamma$ is sufficiently small, then SR is equivalent to the exact ITE, and always converges to the ground state after sufficiently many steps, as long as the initial state is valid.
...
After the above introduction to the general framework of VMC, we discuss some common choices of the variational ansatz in the following.

\section{Variational ansatzes for quantum systems}

\subsection{Jastrow ansatz}

\subsection{Gutzwiller projected state}

\subsection{Neural quantum states (NQS)}

RBM~\cite{carleo2017solving, nomura2021dirac, chen2022systematic}

CNN~\cite{choo2019two}

Hierarchical~\cite{rende2024queries}

\section{Amplitude and phase of quantum ansatz}

To avoid the intricacy of complex-valued functions aforementioned in \cref{sec:cmpl-grad}, especially when using deep neural networks as variational ansatzes, a common practice is to parameterize the amplitude and the phase of the wave function ansatz respectively:
\begin{equation}
\psi(\vs) = \sqrt{q(\vs)} \rme^{\rmi \phi(\vs)},
\end{equation}
where $q(\vs)$ can be interpreted as a probability distribution. It is more common to separate the amplitude and the phase rather than the real and the imaginary parts, because we frequently work with the log-gradient in \cref{eq:vmc-grad-cmpl}, and the ratio of two probabilities in MCMC sampling as in \cref{eq:metropolis}. When sampling the configurations $\vs$, we only need to evaluate $q(\vs)$, and not $\phi(\vs)$. In particular, when parameterizing $q(\vs)$ using the autoregressive (AR) model as in \cref{eq:autoreg}, there is no need for $\phi(\vs)$ to also fulfill the AR property.

This separation of the amplitude and the phase has been applied in abundant literature of NQS~\cite{torlai2018neural, hibat2020recurrent, astrakhantsev2021broken}. In particular, Ref.~\cite{bukov2021learning} provides empirical evidence that an ansatz with this separation achieves higher accuracy than the corresponding ansatz with the complex parameterization, and Ref.~\cite{wang2024variational} even shows that the complex parameterization is outperformed by only optimizing the amplitude and fix the phase according to the Marshall sign rule. On the other hand, there is also considerable literature using the complex parameterization to achieve high accuracies on various systems~\cite{roth2020iterative, li2022bridging, viteritti2023transformer}, which typically use deeper networks and more parameters than the formers. Also, it remains an open question whether there is benefit in sharing a part of the parameters between the amplitude and the phase networks.

Amplitude is hard~\cite{bukov2021learning, park2022expressive}

Sign structure is hard~\cite{westerhout2020generalization, szabo2020neural}

Marshall sign rule is necessary~\cite{carleo2017solving, choo2019two, bukov2021learning, li2022bridging}

Deep neural networks are more prone to sign problem~\cite{szabo2020neural}

Vision transformer, no manual Marshall sign rule

Optimize the phases

Phasor sum~\cite{szabo2020neural}

Two-stage training~\cite{szabo2020neural}

Alternating learning, hysteresis optimization~\cite{astrakhantsev2021broken}

Share parameter or not

% Optimize the signs~\cite{torlai2020wave}

Optimize the signs by cross entropy~\cite{park2022expressive}

It remains an open question whether this can be used in unsupervised learning, with modern relaxation estimators.

Optimize the signs using combinatorial optimization~\cite{westerhout2023many}

Evolutionary algorithm~\cite{chen2022neural}

The understanding is still limited on the relation between the effectiveness of these methods and the properties of the physical system as well as the neural network.
