...
\end{gather}
where $u$ and $v$ are independently sampled from $\calU[0, 1)$, and $\lVert \cdot \rVert$ is the absolute value of the determinant of the matrix. Discrete distributions can be obtained by discretizing continuous distributions. For example, the Bernoulli distribution $p(x) = (1 - p_1) \delta_{x, 0} + p_1 \delta_{x, 1}$ with the parameter $p_1$ can be obtained by \cref{alg:bernoulli}. In this thesis, we refer to this kind of sampling methods as exact sampling.
...
\label{eq:metropolis}
\end{equation}
We accept the new configuration $\vs'$ by this probability, and otherwise reject it and rollback to $\vs$. It can be checked that the target distribution $p(\vs)$ is exactly the equilibrium distribution of the Markov transition matrix:
\begin{gather}
...
\end{equation}
is a metric for MCMC to assess the loss in sample diversity due to autocorrelation. It depends on the complexity of the target distribution, and the method to propose new configurations. In the analytical study of the spin dynamics, it is also known as the relaxation time.
...
\caption[Distribution of energy per site for frustrated plaquette model]{
