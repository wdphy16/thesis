...
The function $\rho_i$ is parameterized by a FFNN, whose inputs are the variables $\{\xi_{i l} \mid l > i\}$ after the first layer.
...
We start from directly factorizing the many-body Boltzmann distribution $p_\text{B}(\vs)$ into the AR conditional distributions in \cref{eq:autoreg}:
\begin{equation}
...
All the RRGs have degree $d = 3$, and are generated from the uniform distribution of all such RRGs using the Steger--Wormald algorithm~\cite{steger1999generating}.
