...
\end{cases},
\end{gather}
where $\odot$ denotes the element-wise multiplication. This masked linear layer is illustrated in \cref{fig:arnn-arch}~(a). Multiple linear layers can be stacked to improve the expressiveness of the network, as shown in \cref{fig:arnn-arch}~(b), where the whole network fulfills the AR property as long as only one layer masks out the ``straight right'' connections $M_{i i}$. This architecture is known as the masked autoencoder for distribution estimation (MADE)~\cite{germain2015made}.
...
An experiment to qualitatively study the ability of ARNN to capture multiple ground states is conducted on the Hopfield model. As discussed in \cref{sec:hopfield}, it is a model of associative memory where one can store and retrieve multiple patterns. We construct a Hopfield model with $N = 100$ spins and $n_\text{pat} = 2$ random orthogonal patterns, and train a dense ARNN that consists of only a single layer. Then we generate $10^4$ samples from the ARNN, and compute their probabilities $q(\vs)$ given by the ARNN, as well as their overlaps $O_1(\vs) = \ip{\vxi^{(1)}}{\vs}$, $O_2(\vs) = \ip{\vxi^{(2)}}{\vs}$ with the two patterns, respectively.

\Cref{fig:arnn-hop} shows that the ARNN successfully samples the memorized patterns with exponentially higher probabilities than other states, in the retrieval phase with high $\beta$. This indicates the capability of ARNN to avoid mode collapse and capture all the modes separated by exponentially high energy barriers in this Hamiltonian.
...
\end{equation}
where $J_{i j}$ are the estimated values and $J^*_{i j}$ are the true values. The results are shown in \cref{fig:arnn-sk}~(b), where the ARNN again outperforms traditional methods by a large margin.
...
\end{equation}
and simplifying the result. When the Hamiltonian has only two-body interactions, such as the Ising model in \cref{eq:cl-ising-general}, it has been derived in Ref.~\cite{biazzo2023autoregressive} that the conditional probability takes the form:
\begin{align}
